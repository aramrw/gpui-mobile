    ...
    if let Some(mut entries) = fs::read_dir(dir).await.log_err() {
        while let Some(entry) = entries.next().await {
            if let Some(entry) = entry.log_err() {
                ...
                let Ok(metadata) = fs::metadata(&entry_path).await else {
                    panic!("could not get metadata in util/src/fs.rs");
                };

                match predicate(entry_path.as_path())
                {
                    true => {
                        if metadata.is_file() {
    ...
    if let Some(mut entries) = fs::read_dir(dir).await.log_err() {
        while let Some(entry) = entries.next().await {
            if let Some(entry) = entry.log_err() {
                ...
                {
                    Some(file_name) if predicate(&file_name) => return Some(entry_path),
                    _ => (),
                }
            }
        }
