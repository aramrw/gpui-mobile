...
pub fn replace_subschema<T: JsonSchema>(
    ...
    schema: impl Fn() -> schemars::schema::Schema,
    ...
    assert!(definitions.contains_key(schema_name.as_str()));
    definitions.insert(schema_name.to_string(), schema());
    schemars::schema::Schema::new_ref(format!("{DEFS_PATH}{schema_name}"))
}
...
pub fn add_new_subschema(
    ...
    schemars::schema::Schema::new_ref(format!("{DEFS_PATH}{name}"))
}
...
impl schemars::transform::Transform for DefaultDenyUnknownFields {
    fn transform(&mut self, schema: &mut schemars::schema::Schema) {
